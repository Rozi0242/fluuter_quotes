...
import 'package:quotes_flutter/model/quote.dart';
import 'dart:async';
import 'dart:convert';
...
Future<List<Quotes>> fetchQuotes(http.Client client) async {
  final urlApi = "https://quotesondesign.com/wp-json/wp/v2/posts/?orderby=rand";
  final response = await client.get(urlApi);
  if (response.statusCode == 200) {
    return compute(parseQuotes, response.body);
  } else {
    throw Exception("Failed reload Quotes");
  }
}
...
class _QuotesPageState extends State<QuotesPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Container(
        padding: EdgeInsets.only(top: height / 12, bottom: height / 5.5),
          ...
          children: [
            ...
            FutureBuilder<List<Quotes>>(
              ...
              builder: (context, snapshot) {
                ...
                if (snapshot.hasData) {
                  return _buildQuotesSection(width, height, snapshot.data);
                } else {
                  return Center(
